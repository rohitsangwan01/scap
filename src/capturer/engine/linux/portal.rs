...
impl Stream {
    ...
    #[allow(unused)]
    pub fn stream_dict(&self) -> StreamVardict {
        self.1.clone()
    }

    pub fn from_dbus(stream: &Variant<Box<dyn RefArg>>) -> Option<Self> {
        let mut outer_iter = stream.as_iter()?.next()?.as_iter()?;
        let mut list = outer_iter.next()?.as_iter()?;
        let node_id = list.next()?.as_u64()?;
        let mut props = StreamVardict {
            id: None,
            position: None,
            size: None,
            source_type: None,
                        ...
                        match key {
                            ...
                            "position" => {
                                if let Some(mut tup) = entry.as_iter() {
                                    if let Some(value) = tup.next() {
                                        if let Some(mut data) = value.as_iter() {
                                            let x = data.next()?.as_i64()? as i32;
                                            let y = data.next()?.as_i64()? as i32;
                                            props.position = Some((x, y));
                                        }
                                    }
                                }
                            }
                            ...
                            _ => {
                                // Other keys (node_id, flags, etc.) ignored
                                // println!("Unhandled stream key: {}", key);
                            }
                        }
                        last_id = None;
                    } else {
                        last_id = entry.as_str();
                    }
                }
            }
        }

        Some(Self(node_id as u32, props))
    }
}
...
impl<'a> ScreenCastPortal<'a> {
    ...
    fn create_session(&self) -> Result<String, LinCapError> {
    ...
    pub fn create_stream(&self) -> Result<Stream, LinCapError> {
        let session_handle = dbus::Path::from(self.create_session()?);
